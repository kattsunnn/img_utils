...
def glob_file_paths(dir_path: str, pattern: str="*.txt*" ) -> List[str]:
...
def load_imgs(dir_path):
    ...
    if os.path.isfile(dir_path):
        ...
        img_paths = glob_img_paths(dir_path)
        imgs = [cv2.imread(img_path) for img_path in img_paths ]
        return imgs
    
    else:
        raise ValueError(f"入力パスが存在しません: {dir_path}")
